...
def register_best_model(
    model_name: str, experiment_name: str, metric: str, stage: str = "Production"
):
    client = MlflowClient()
    ...
    mlflow.register_model(model_uri=f"runs:/{best_run_id}/model", name=model_name)
...
def load_registered_retrieval_model(
    model_name: str, stage: str = "Production", data_loader: DataLoader | None = None
) -> tf.keras.Model:
    client = MlflowClient()
    ...
    logger.info(
        f"Latest version of {model_name} model in Production stage: {registered_model.version}"
    )
    ...
    retrieval_model = init_retrieval_model_checkpoint(
        model_base_path=logged_model_base_path,
...
def load_registered_predictor_model(
